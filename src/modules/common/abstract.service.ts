...
export abstract class AbstractService {
  ...
  async findById(id: string, relations = []): Promise<any> {
    try {
      ...
      if (!element) {
        throw new BadRequestException('Cannot find an element with id: ' + id)
      }
      return element
    } catch (error) {
      Logging.error(error)
      throw new InternalServerErrorException('Something went wrong while searching for an id: ' + id)
    }
  }
  ...
  async paginate(page = 1, relations = []): Promise<PaginatedResult> {
    ...
    try {
      ...
      Logging.error(error)
      throw new InternalServerErrorException('Something went wrong while searching for the paginated elements: ')
    }
  }
}
